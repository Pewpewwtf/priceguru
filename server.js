...
import {
  ...
  getAllItems,resetAndImport,usingPostgres
} from './db.js';
...
app.get('/api/health',async(req,res)=>res.json({ok:true,version:'9.1',database:usingPostgres()?'postgres':'memory',time:new Date().toISOString()}));
app.get('/api/state',async(req,res,next)=>{try{res.json({ok:true,...await getState()});}catch(e){next(e);}});
app.get('/api/export',async(req,res,next)=>{try{const state=await getState();res.setHeader('content-disposition','attachment; filename="pricewatch-v9.1-backup.json"');res.json(state);}catch(e){next(e);}});
...
await initDb();
app.listen(PORT,'0.0.0.0',()=>console.log(`PriceWatch v9.1 listening on :${PORT} (${usingPostgres()?'PostgreSQL':'memory mode'})`));
